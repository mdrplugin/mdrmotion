...
//importfolder = importfolder.selectDlg ("Select a folder pls");
...
// fünfte reihe der gui
var groupFive = mainWindow.add("group", undefined,"groupFive");
groupFive.orientation = "row";
// loc box und buttons
var fileLocBox = groupFive.add("edittext",undefined,"Selected File Location");
fileLocBox.size = [160,20];
var getFileButton = groupFive.add("button",undefined,"File...");
getFileButton.helpTip = "Select a ... File";

// sechste reihe der gui
var groupSix = mainWindow.add("group",undefined,"groupSix");
groupSix.orientation="row";
var applyButton = groupSix.add("button", undefined, "Apply");

// siebte reihe der gui
var groupSeven = mainWindow.add("group",undefined,"groupSeven");
groupSeven.orientation="row";
var importVersuch = groupSeven.add("button", undefined, "importVersuch");
...
//----------------------------------------------------------------------------------------------------------------------------------
/*
importierButton.onClick = function filterFiles(files, extension){
        var filtered = [];
...
getFileButton.onClick = function() {
    file = file.openDlg("pen a file pls","Acceptable Files:*aep");
    fileLocBox.text = file.fsName;
    check = 1; // datei wurde ausgewählt
    }

applyButton.onClick = function importFiles(files){
    app.beginSuppressDialogs();
    for(var i =0; i<files.length;i++){
            app.project.importFile(new ImportOptions(files[i]));
        }
...
/*
importVersuchButton.onClick = function (){

var importante = new ImportOptions(File("c:\Users\gph-malen\Desktop\file list\Untitled Project.aep"));
    if importante.canImportAs(ImportAsType.COMP) {
        importante.importAs = ImportAsType.COMP;
    }
return importante;
}*/
...
importVersuchButton.onClick = function(){
    ...
    importFile = importFile.openDlg("Choose Mask Text File", "Text File:*.aep");
   
    var fpath = importFile.absoluteURI;
    readDocument(importFile);
   
    var newComp = project.items.addComp(File("C:\Users\gph-malen\Desktop\file list\Untitled Project.aep"));
